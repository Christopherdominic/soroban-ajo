        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            {activeSection === 'overview' && (
              <>
                <ProfileCard profile={profile as any} isLoading={isLoading} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <StatCard
                    title="Total Payouts"
                    value={`$${(profile.stats as any).totalPayouts?.toFixed(2) || '0.00'}`}
            ...
            {activeSection === 'edit' && (
              <ProfileForm profile={profile as any} onSave={updateProfile as any} isLoading={isLoading} />
            )}

            {activeSection === 'settings' && (
              <SettingsPanel preferences={profile.preferences as any} onSave={savePreferences as any} />
            )}

            {activeSection === 'activity' && (
              <ActivityHistory activities={activities as any} isLoading={isLoading} />
            )}
          </div>
        </div>
      </div>
...
const ActivityItem: React.FC<ActivityItemProps> = ({ activity }) => {
  ...
  return (
    <div className="flex items-start gap-4 p-4 hover:bg-gray-50 dark:hover:bg-slate-700/50 rounded-lg transition-colors">
      ...
      <span className={`text-xs px-2 py-1 rounded-full ${activity.status === 'completed' ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' :
